...
macro "Build Nonmotorized Networks" (Args)
    ...
    RunMacro("Build NM network", Args.[Road Line Layer], "Walk", "DTWB contains 'W'", "WalkTime", walknet)

    AppendToLogFile(1, "Build bike network")
    bikenet = RunMacro("Join Path", {Args.[Output Folder], "bike.net"})
    RunMacro("Build NM network", Args.[Road Line Layer], "Bike", "DTWB contains 'B'", "BikeTime", bikenet)
